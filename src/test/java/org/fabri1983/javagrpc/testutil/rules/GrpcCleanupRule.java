...
/**
 * A JUnit {@link TestRule} that can register gRPC resources and manages its
 * automatic release at the end of the test. If any of the resources registered
 * to the rule can not be successfully released, the test will fail.
 *
 * @since 1.13.0
 */
@ExperimentalApi("https://github.com/grpc/grpc-java/issues/2488")
@NotThreadSafe
public class GrpcCleanupRule implements TestRule {
	...
	/**
	 * Sets a positive total time limit for the automatic resource cleanup. If any
	 * of the resources registered to the rule fails to be released in time, the
	 * test will fail.
	 *
	 * <p>
	 * Note that the resource cleanup duration may or may not be counted as part of
	 * the JUnit {@link org.junit.rules.Timeout Timeout} rule's test duration,
	 * depending on which rule is applied first.
	 *
	 * @return this
	 */
	...
	/**
	 * Registers the given channel to the rule. Once registered, the channel will be
	 * automatically shutdown at the end of the test.
	 *
	 * <p>
	 * This method need be properly synchronized if used in multiple threads. This
	 * method must not be used during the test teardown.
	 *
	...
	/**
	 * Registers the given server to the rule. Once registered, the server will be
	 * automatically shutdown at the end of the test.
	 *
	 * <p>
	 * This method need be properly synchronized if used in multiple threads. This
	...
	private void teardown() {
		...
		for (int i = resources.size() - 1; i >= 0; i--) {
			...
			try {
				boolean released = resources.get(i)
						.awaitReleased(timeoutNanos - stopwatch.elapsed(TimeUnit.NANOSECONDS), TimeUnit.NANOSECONDS);
				if (!released) {
